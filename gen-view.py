...
CHAPTERS = ["Hardware Software Interface", "Intro Computer Architecture", "Memory Layout", "Memory Security"]
LABS_PATH = "chapters/"
...
def group_reading():
    ...
    for root, _, files in os.walk(LABS_PATH):
        for f in files:
...
def hypenate(text: str) -> str:
    """Converts a string into a hyphenated lowercase format."""
    return "-".join(text.strip().split(" ")).lower()
...
def find_broken_links():
    ...
    """
    prefixes = [ "media", "tasks",  "reading", "guides", "http"]
...
def main():
    """
    1. Group all media, and tasks in their respective folders in .view.
    Will result in .view/media/, .view/tasks/

    2. For each lab, concatenate the lab content and tasks.

    3. For each lab, solve relative links to media, tasks.
